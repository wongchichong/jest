...
export type Unshift<T extends any[]> = T extends [any, ...infer U] ? U : never;

// export type RemoveFirstParameter<T extends <A, B, C, D, E, F, G>(a: A, b: B, c: C, d: D, e: E, f: F, g: G) => any> =
//   (...args: Unshift<Parameters<T>>) => ReturnType<T>;

export type RemoveFirstParameter<T> =
  //  T extends <A, B, C, D, E, F, G>(a: A, b: B, c: C, d: D, e: E, f: F, g: G) => any ? never : 
  T extends (a: any, b: infer B) => any ? (b: B) => ReturnType<T> :
  T extends (a: any, b: infer B, c: infer C) => any ? (b: B, c: C) => ReturnType<T> :
  T extends (a: any, b: infer B, c: infer C, d: infer D) => any ? (b: B, c: C, d: D) => ReturnType<T> :
  T extends (a: any, b: infer B, c: infer C, d: infer D, e: infer E) => any ? (b: B, c: C, d: D, e: E) => ReturnType<T> :
  T extends (a: any, b: infer B, c: infer C, d: infer D, e: infer E, f: infer F) => any ? (b: B, c: C, d: D, e: E, f: F) => ReturnType<T> : never;
// T extends <C>(a: any, b: any, c: C) => any ? <B, C>(b: B, c: C) => ReturnType<T> :never

// export type RemoveFirstParameter<T> = T extends ((_: any, p: infer P) => infer R) ? (p: P) => R : never;
...
export interface BaseExpect {
  ...
  extend: <T extends Record<string, (...args: any[]) => any>, E>(matchers: T, isInternal: boolean, expect: Expect<E>) => Expect<WrappedType<T>>;
...
export type Expect<M = unknown> = {
  <T = unknown>(actual: T): M & //Matchers<void, T>
    Inverse<M>; //&
  // PromiseMatchers<T>;
} & BaseExpect &
  AsymmetricMatchers &
